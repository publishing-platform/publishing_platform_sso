...
RSpec.describe PublishingPlatform::SSO::Railtie do
  let(:cache) { double(:cache) }

  it "re-uses the Rails cache" do
    expect(PublishingPlatform::SSO::Config.cache).to eq Rails.cache
  end

  it "honours API only setting" do
    expect(PublishingPlatform::SSO::Config.api_only).to eq false
  end
  ...
  describe "configuring intercept_401_responses" do
    it "sets warden intercept_401 to false when the configuration option is set to false" do
      allow(PublishingPlatform::SSO::Config).to receive(:intercept_401_responses).and_return(false)
    ...
    it "sets warden intercept_401 to true when the configuration option is set to true" do
      allow(PublishingPlatform::SSO::Config).to receive(:intercept_401_responses).and_return(true)
