...
RSpec.describe Warden::SessionSerializer do
  before :each do
    @old_user_model = PublishingPlatform::SSO::Config.user_model
    PublishingPlatform::SSO::Config.user_model = SerializableUser
    @user = double("SerializableUser", uid: 1234)
    @serializer = Warden::SessionSerializer.new(nil)
  end
  after :each do
    PublishingPlatform::SSO::Config.user_model = @old_user_model
  end
  ...
  describe "deserialize a user" do
    it "should return the user if the timestamp is current and a Time" do
      expect(SerializableUser).to receive(:where).with(uid: 1234).and_return(double(first: :a_user))

      result = @serializer.deserialize [1234, Time.now.utc - PublishingPlatform::SSO::Config.auth_valid_for + 3600]
    ...
    it "should return the user if the timestamp is current and is an ISO 8601 string" do
      ...
      result = @serializer.deserialize [1234, (Time.now.utc - PublishingPlatform::SSO::Config.auth_valid_for + 3600).iso8601]
