...
RSpec.describe "Authenication and authorisation" do
  context "omniauth request phase" do
    ...
    before do
      visit "/auth/publishing_platform"
    end
  ...
  context "omniauth callback phase" do
    it "includes pkce code_verifier in request for /oauth/token" do
      visit "/auth/publishing_platform"
      ...
      stub_request(:post, "http://signon/oauth/token")

      visit "/auth/publishing_platform/callback?state=#{state}"

      expect(WebMock).to have_requested(:post, "http://signon/oauth/token")
  ...
  context "when accessing a route that requires authentication" do
    it "redirects an unauthenticated request to signon" do
      ...
      visit "/restricted"
      expect(page.response_headers["Location"]).to match("/auth/publishing_platform")
    ...
    # TODO: - uncomment this test when remotely signed out functionality is implemented
    ...
    it "restricts access if a user is authenticated but session has expired" do
      ...
      Timecop.travel(Time.now.utc + PublishingPlatform::SSO::Config.auth_valid_for + 5.minutes) do
        visit "/restricted"
        expect(page.status_code).to eql(302)
        expect(page.response_headers["Location"]).to match("/auth/publishing_platform")
      end
    ...
    it "restricts access when given an invalid bearer token" do
      stub_request(:get, "http://signon/user.json?client_id=publishing-platform-sso-test")
    ...
    it "returns a 401 when a bearer token is missing and the app is api_only" do
      allow(PublishingPlatform::SSO::Config).to receive(:api_only).and_return(true)
  ...
  def stub_signon_authenticated(permissions: [])
    ...
    visit "/auth/publishing_platform/callback?code=code&state=#{state}"
  end
